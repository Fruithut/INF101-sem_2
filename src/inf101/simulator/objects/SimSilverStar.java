 ...
 */
public class SimSilverStar extends AbstractSimObject implements IEdibleObject {
    ...
    /**
     * Constructs a SimSilverStar object
     ...
     */
    public SimSilverStar(Position pos, double size, int expiration) {
    ...
    public void draw(GraphicsContext context) {
        ...
        if (expirationTimer - counter < 550 && expirationTimer - counter >= 350) {
            context.drawImage(MediaHelper.getImage("star_silver_slow.gif"),0,0,getWidth(),getHeight());
        } else if (expirationTimer - counter < 350) {
            context.drawImage(MediaHelper.getImage("star_silver_fast.gif"),0,0,getWidth(),getHeight());
        } else {
            context.drawImage(MediaHelper.getImage("star_silver.png"),0,0,getWidth(),getHeight());
        }
    }
     ...
     * @param silverStar under consideration
     */
    private static void checkState(SimSilverStar silverStar) {
        if (silverStar.expirationTimer <= 0)
            throw new IllegalArgumentException("The expiration value must be larger than 0: " + silverStar.expirationTimer);
        if (silverStar.size <= 0)
            throw new IllegalArgumentException("The size of the SimGoldStar must be larger than 0: " + silverStar.size);
    }
